...
design_matrix <- read.table("blancgrp_266225_RNAseq_monosome.design.txt", header=TRUE, row.names=1)
df <- design_matrix
...
total_RNAseq <-  fread("blancgrp_266225_RNAseq_monosome.counts.raw.txt", header = TRUE) |>
...
exp.common.genes <- intersect(rownames(mtx_count_mono),rownames(mtx_count_poly))
exp.common.genes.pc <- sort(intersect(gene_lengths_pc$Gene, exp.common.genes))


mtx_count_mono.pc <- mtx_count_mono[exp.common.genes.pc,]
mtx_count_poly.pc <- mtx_count_poly[exp.common.genes.pc,]
...
## TGFb_Vs_Unt == rpkm Poly /(rpkm poly + rpkm mono)
## TGFb_Vs_Unt_1 == rpkm Poly / rpkm mono
## matrix_davide <- log2(mtx_count_poly.pc+1/(mtx_count_mono.pc + mtx_count_poly.pc)+1)
xl_list <- list('TGFb_Vs_Unt'= topTable_tgfb_vs_unt, 'CX5461_Vs_TGFb'= topTable_tgfbCX_vs_tgfb, 'TGFb_Vs_Unt_1'= topTable_tgfb_vs_unt_1, 'CX5461_Vs_TGFb_1'= topTable_tgfbCX_vs_tgfb_1, 
                'TGFb_Vs_Unt_davide'= topTable_tgfb_vs_unt_2, 'CX5461_Vs_TGFb_davide'= topTable_tgfbCX_vs_tgfb_2)
write.xlsx(xl_list, file = "Translational efficiency in Monosome RNAseq Vs Polysome RNAseq profiling.xlsx", rowNames=TRUE)


################################################################################
################################################################################
## Polysome Vs. Monosome+Polysome
################################################################################
################################################################################
...
save.image("translational_eficiency_monosome_vs_polysome.Rdata")
